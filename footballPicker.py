...
def getMyBookie(teamDict):
    ...
    for i in range(len(divs)):
        # print(teams[i].text, divs[i].text)
        if divs[i]["data-wager-type"] == "ml" and divs[i]["data-sport-id"] == "NFL":
            teams0.append([divs[i]["data-team"], divs[i]["data-odds"]])
...
def getDraftkings(teamDict):
    page0 = requests.get("https://sportsbook.draftkings.com/leagues/football/88670561?category=game-lines&subcategory=game", headers=headers)
